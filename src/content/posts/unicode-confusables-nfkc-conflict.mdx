...
This isn't a single edge case. We found 31 characters where confusables.txt and NFKC disagree:
...
Rather than hand-curating a confusable map (which becomes stale when Unicode ships new versions), we wrote a generator script that:
