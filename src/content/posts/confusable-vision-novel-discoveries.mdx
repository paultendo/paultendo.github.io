...
Milestone 2 of [confusable-vision](https://github.com/paultendo/confusable-vision) scans every Unicode character that meets three criteria: identifier-safe (General Category Letter or Number), not already in confusables.txt, and not CJK/Hangul (too structurally distinct from Latin to produce meaningful matches). That gives 23,317 candidates.

Not every candidate has font coverage. Fontconfig found at least one system font for 12,555 characters (53.8%). The remaining 10,762 have no installed font that contains them -- they cannot be rendered and therefore cannot be scored. The covered candidates average 7.1 fonts each.
...
3. **Historical and minority scripts.** Coptic, Gothic, Old Hungarian, Lydian -- scripts with small user populations but valid identifier characters and active Noto Sans font coverage.
...
Next: cross-script validation (M2b) and weighted scoring for [namespace-guard](https://github.com/paultendo/namespace-guard) (M3).

---

*[confusable-vision](https://github.com/paultendo/confusable-vision) is MIT-licensed. The discovery data is CC-BY-4.0. The full candidate analysis, 96-script breakdown, and all render artifacts are in the repo. [namespace-guard](https://github.com/paultendo/namespace-guard) (v0.15.1, zero dependencies, MIT) will integrate these discoveries in a future release.*
