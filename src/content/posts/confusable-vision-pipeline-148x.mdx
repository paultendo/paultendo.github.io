...
This post documents optimisations to the SSIM (pixel rendering) pipeline. [RaySpace](/posts/rayspace-methodology/) subsequently replaced pixel rendering entirely with vector-outline raycasting, comparing font outlines as geometry rather than rasterised images. The optimisations below are no longer part of the active pipeline, but the profiling methodology and Sharp/WASM findings remain relevant to anyone doing image-heavy Node.js work.
