...
The bigram pipeline searched 190 million combinations (676 Latin bigrams x 294,646 bank targets x 245 fonts) and found **571,753 unique multi-character confusable pairs**. No previous system attempted this search. The results include cross-script bigram confusables where a Latin two-character sequence resembles a single character from another script:
...
**Latin "oy" matches Cyrillic ѹ (uk, U+0479)** at distance 0.000 in Helvetica, confirmed across 16 fonts. This one is geometrically perfect but practically harmless: U+0479 has `IdentifierType=Obsolete` and cannot appear in identifiers or domain names. It illustrates the kind of exotic match the pipeline surfaces, and why [IDN relevance filtering](/posts/idn-relevance/) matters.
...
On the single-character side, the enriched five-layer signature recovered **19,983 cross-script pairs** that the earlier count-only raycasting filter had been incorrectly rejecting. These all passed SDF scoring when tested directly. The count-only filter was too aggressive; the additional layers (positions, crossing angles, pings) preserved the structural signal needed to identify them.
