...
This post uses SSIM-scored per-font data from the pixel rendering pipeline. [RaySpace](/posts/rayspace-methodology/) now provides per-font continuous distance scores across 245 fonts using vector-outline raycasting. The concept of font-specific confusable maps remains the same, but the underlying scores are now geometric rather than pixel-based.
